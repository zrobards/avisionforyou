...
export default function AdminContactPage() {
  ...
  const filteredInquiries = filter === 'all'
    ? safeInquiries
    : safeInquiries.filter(inq => inq.status === filter)

  const statusCounts = {
    all: safeInquiries.length,
    NEW: safeInquiries.filter(i => i.status === 'NEW').length,
    IN_PROGRESS: safeInquiries.filter(i => i.status === 'IN_PROGRESS').length,
    RESPONDED: safeInquiries.filter(i => i.status === 'RESPONDED').length,
    RESOLVED: safeInquiries.filter(i => i.status === 'RESOLVED').length
  }
