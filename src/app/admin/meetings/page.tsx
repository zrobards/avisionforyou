...
import { Search, Plus, Filter, X, Calendar, Users, Clock, MapPin, Edit2, Trash2, ChevronDown, Download } from 'lucide-react'
...
export default function AdminMeetingsPage() {
  ...
  const handleEditMeeting = (meeting: Meeting) => {
    setFormData({
      ...
      link: meeting.link || '',
      capacity: meeting.capacity?.toString() || ''
    })
  ...
  const exportRsvpsToCSV = (meeting: Meeting) => {
    if (!meeting.rsvps || meeting.rsvps.length === 0) {
    ...
    const rows = meeting.rsvps.map(rsvp => [
      rsvp.user.name || 'N/A',
      rsvp.user.email || 'N/A',
      rsvp.status,
      new Date().toLocaleDateString() // You'd get this from RSVP createdAt if available
    ])
    ...
    a.href = url
    a.download = `${meeting.title.replace(/[^a-z0-9]/gi, '_')}_RSVPs_${new Date().toISOString().split('T')[0]}.csv`
    document.body.appendChild(a)
    a.click()
    document.body.removeChild(a)
    window.URL.revokeObjectURL(url)

    showToast(`Exported ${rows.length} RSVPs`, 'success')
  }
          ...
          <div className="bg-white rounded-lg border border-gray-200 shadow-sm p-6 animate-slide-down">
            ...
            <form onSubmit={handleSaveMeeting} className="space-y-4">
              ...
              <div>
                ...
                />
                <p className="text-xs text-gray-500 mt-1">Maximum number of RSVPs allowed</p>
              </div>
                      ...
                      <span className="flex items-center gap-1 cursor-pointer hover:text-brand-purple" onClick={() => setExpandedRsvps(expandedRsvps === meeting.id ? null : meeting.id)}>
                        ...
                        {meeting.rsvps && meeting.rsvps.length > 0 && (
                          <button
                            onClick={(e) => {
                              e.stopPropagation()
                              exportRsvpsToCSV(meeting)
                            }}
                            className="ml-2 p-1 hover:bg-brand-purple/10 rounded"
                            title="Export RSVPs to CSV"
                          >
